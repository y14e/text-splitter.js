...
class TextSplitter {
  ...
  private nobr(node: HTMLElement = this.dom): void {
    if (node.nodeType === 3) {
  ...
  private split(by: 'word' | 'char', node: HTMLElement = this.dom): void {
      ...
      if (node.nodeType === 3) {
        ...
        segments.forEach(segment => {
          ...
          [by, segment.segment.charCodeAt(0) === 32 && 'whitespace'].filter(Boolean).forEach(type => {
            element.dataset[type as string] = type !== 'whitespace' ? text : '';
          });
